  ...
  return (
    <WorkerLayout>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => setShowForm(true)}><Plus size={16} />{t('addService')}</Button>
      </div>

      {showForm && (
        <Card className="mb-4 border-primary-200">
          <CardBody className="space-y-4">
            <h3 className="font-semibold text-gray-900">{t('newService')}</h3>
            <Input label={t('serviceNameLabel')} value={name} onChange={(e) => setName(e.target.value)} placeholder={t('serviceNamePlaceholder')} />
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-gray-700">{t('serviceDescLabel')}</label>
              <textarea rows={2} value={desc} onChange={(e) => setDesc(e.target.value)} placeholder={t('serviceDescPlaceholder')}
            ...
            <Input label={t('servicePriceLabel')} type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="15000" />
          ...
          <Card key={s.id}>
            <CardBody className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <button className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg"><Pencil size={16} /></button>
                <button onClick={() => setServices((prev) => prev.filter((x) => x.id !== s.id))} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={16} /></button>
              </div>
            </CardBody>
          </Card>
        ))}
        {services.length === 0 && (
          <Card>
            <CardBody className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-1">{t('noServicesSub')}</p>
            </CardBody>
          </Card>
        )}
      </div>
